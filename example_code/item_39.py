...
"""
这条军规的目的是通过 @classmethod 来实现灵活且多态的对象构造，
避免把所有构造逻辑堆在 __init__ 方法里。
通过 @classmethod，我们可以根据不同的数据来源或需求，
定义多个构造方法，并且让子类可以通过继承和重写这些方法来支持多态。

简化 __init__ 方法：避免在 __init__ 中堆积复杂的构造逻辑，使得构造方法职责单一。
扩展性强：通过 @classmethod，可以轻松添加更多的构造方法，而无需修改现有代码。
支持继承和多态：子类可以继承和重写这些类方法，实现自定义的构造方式，灵活支持不同的数据输入。
代码更加 Pythonic：这种方法使代码更简洁、灵活，符合 Python 的设计哲学。

总结：
明白了问题在于__init__只能有一个，所以针对不同构造场景来说，就需要进行分流处理，会很复杂，
而通过@classmethod就派生了多个类似_init_的功能从而进行了，构造方法的多态，
每个只关注自己的构造，而__init__也其实只是其中之一，没有之前分流处理烦恼
"""
