...
军规 21: Know How Closures Interact with Variable Scope
军规 21: 理解闭包如何与变量作用域交互
闭包的目的是通过作用域链来封装状态并保持外部变量的引用，
它解决的是如何在不同函数间共享状态的问题，而不只是简单的限制访问。
访问修饰符则是用来控制类或模块中的访问权限，这与闭包的设计意图不同。
什么时候使用闭包：
当你需要持久化某些状态，但不希望用全局变量或类来实现时，闭包是一个很好的选择。
当函数需要动态创建，但依然希望每个创建的函数能共享或保持某些外部数据时，闭包提供了优雅的解决方案。
需要保持数据的私有性，即不希望外部直接访问数据，闭包可以实现这种数据封装。

闭包替代方案：
虽然闭包非常灵活，但如果场景更复杂（比如需要大量状态管理和行为），可能使用类和对象会更合适。
类和对象可以显式地管理状态、行为和数据，且结构化更明确。

总结：
闭包并不是每个场景都需要用到，但在涉及状态封装、回调函数、工厂函数时，闭包是一种非常自然和简洁的解决方案。
如果你需要保持状态、处理私有数据或构建动态逻辑，闭包可以为你提供一种有效的编程模式。
"""
